...
<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
		...
		<?php endif; ?>
		<p class="freshness"><a href="<?php echo get_permalink( $post->ID ); ?>revisions/" title="<?php esc_attr_e( date( get_option('date_format') . ' ' . get_option('time_format'), strtotime($post->post_modified)) ) ?>" class="updated">Updated <?php echo dokumento_human_time_diff( 1, strtotime($post->post_modified) ); ?> ago by <span class="author"><?php the_modified_author(); ?></span></a></p>
